		...
		try { // try
			...
			while ((strLine = br.readLine()) != null) {
				...
				case 0: // dewnotset
					...
					//printC("load dewnotset " + dslb.dewnotsetmax + " = " + strLine);
					continue;
				...
				//	printC("load dewinventoryblock " + dslb.dewinventoryblockmax + " = " + strLine);
					continue;
				default:
				//printC("ptdew&dewdd : error loading " + filena + " cuz moden == -1");
					return false;
				}
			...
			System.err.println("Load dewset list block file Error: " + e.getMessage());
			return false;
		} // catch
...
public boolean isdewinventoryblock(int blockid) {
	...
	public void showdewsetlistblock(Player p) {
		...
		if (isdewset(a1) == false) {
			p.sendMessage("this item my plugin not allow for dewset " + a1 + ":" + a2);
			return;
		}
	}
}
