...
public abstract class dewsetdatabase {
	...
	public boolean isdewinventoryblock(int blockid) {
		...
		for (int i = 0; i < dslb.dewinventoryblockmax; i++) {
			if (blockid == dslb.dewinventoryblock[i]) return true;
		}
		return false;
	}
	...
	public boolean isdewset(int blockid) {
		...
		for (int i = 0; i < dslb.dewnotsetmax; i++) {
			if (blockid == dslb.dewnotset[i]) return false;
		}

		Material gid = Material.getMaterial(blockid);
		if (gid == null) return false;

		if (gid.isBlock() == false) return false;
	...
	public boolean loaddewsetlistblockfile() {
		...
		try { // try
			...
			dprint.r.printC("ptdew&dewdd: " + tr.gettr("starting_loading_file")
					+ filena);
			...
			System.err.println(tr.gettr("Error_loading_file") + " : "
					+ e.getMessage() + " " + filena);
			return false;
		} // catch
	...
	public void showdewsetlistblock(Player p) {
		...
		if (isdewset(a1) == false) {
			p.sendMessage(tr.gettr("this_item_not_allow_for_dewset") + a1 + ":"
					+ a2);
			return;
		}
	}
